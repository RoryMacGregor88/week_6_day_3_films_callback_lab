...
describe('Cinema', function () {
  ...
  it('should be able to find a film by title', function() {
    const actual = cinema.findFilmByTitle(films, 'Moonlight' )
    assert.strictEqual(actual, moonlight)
  });

  it('should be able to filter films by genre', function() {
    const actual = cinema.filterFilmsByGenre( films, 'drama' )
  ...
  it('should be able to check whether there are no films from a particular year', function() {
    const actual = cinema.filterByYear(films, 2005)
    assert.deepStrictEqual(actual, [])
  });
  it('should be able to check whether all films are over a particular length', function() {
    const actual = cinema.checkAllFilmsLength(films, 90)
    assert.deepStrictEqual(actual, true)
  });

  it('should be able to calculate total running time of all films', function() {
    const actual = cinema.calculateTotalFilmTimes(films)
    assert.deepStrictEqual(actual, 622)
  });

  it('should be able to filter films by property', function() {
    const actual = cinema.filterFilmsByProperty( films, 'drama' )
    assert.deepStrictEqual(actual, [moonlight, trainspotting])
  });
